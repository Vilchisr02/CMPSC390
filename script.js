...
document.querySelector(".sign-up-form").addEventListener("submit", async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById("signUpPopup").style.display = "none"; // Close the signup popup
        } else {
            // If there's an error, show the message returned by the server
            alert(data.message || "Signup failed");
        }
    } catch (error) {
        // If there's an error with the request, log it and alert the user
